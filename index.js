...
function cumlelereDonustur(cumlelerArr, sp = ",") {
  const yeniCumleler = cumlelerArr.map((c) => c.join(sp));
...
function paragrafOlustur(cumlelerArr, cumleKurFN, cumlelereDonusturFN) {
  const cumleDizisi = cumlelereDonusturFN(cumlelerArr, " ");
  const paragraf = cumleKurFN(
    cumleDizisi[1].trim(),
    cumleDizisi[3].trim(),
    cumleDizisi[5].trim(),
    cumleDizisi[7].trim(),
...
console.log("Çıkarılan Meyve 1: ", meyveler.pop());
console.log("Çıkarılan Meyve 2: ", meyveler.shift());
console.log("Kalan Meyveler: ", meyveler);
...
sebzeler.unshift("🐇");
console.log("Tavşan Saldırdı: ", sebzeler);
sebzeler.push("🦔");
console.log("Kirpi Saldırdı: ", sebzeler);
